        ...
        case 'checkout.session.completed': {
          try {
            ...
            if (session.subscription && session.customer) {
              ...
              // Ajoute la logique annuelle : si interval = 'year', calcule la vraie date de fin
              let currentPeriodEnd = new Date(stripeSubscription.current_period_end * 1000)
              const interval = session.metadata?.interval || stripeSubscription.items.data[0]?.price.recurring?.interval
              if (interval === 'year') {
              ...
              // Trial logic: start trial if eligible (jamais eu de trial, pas actif)
              if (!user?.hasTrialUsed && !user?.isTrialActive) {
                const trialService = new TrialService()
          ...
          try {
            ...
            if (interval === 'year') {
              if (subscription.current_period_start) {
                ...
                // Try to fetch from full subscription
                const fullSubscription = await stripe.subscriptions.retrieve(subscriptionId)
                if (fullSubscription.current_period_start) {
                  startDate = new Date(fullSubscription.current_period_start * 1000)
                } else if (currentPeriodEnd) {
                  // Fallback: estimate start date as one year before currentPeriodEnd
